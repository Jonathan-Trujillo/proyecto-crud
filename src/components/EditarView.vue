<template>
  <div>
    <v-container>
      <form style=" gap:0px;margin-top:60px;" v-on:submit.prevent="actualizarDatos">
        <v-container style="display:grid;grid-template-columns: auto auto auto auto auto; ">
          ...
          <v-container>
            <div> Edad: </div>
            <input type="text" id="edad" name="edad" class="rounded grey lighten-3 pl-4" required maxlength="45"
        ...
        <v-container style="display:flex;justify-content:right;gap:20px;width:100%; text-align: center;"> <button type="submit"
            style="background:#1976D2;color:#fff;border-radius:50px;text-decoration:none;font-weight:500; padding: 5px 20px;">Actualizar</button>
...
myDate() {
    ...
    document.getElementById('edad').value = getAge(g);
